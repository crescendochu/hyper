var App = function (arWorldRoot, trackingBackend) {
  ...
  // create demo scenes
  if (
    _this.trackingBackend === "artoolkit" ||
    _this.trackingBackend === "aruco"
  ) {
    _this._arAppletBuilder = new THREEx.ARAppletBuilder(
      camera,
...
App.prototype.selectApplet = function (arScene) {
  ...
  [].slice
    .call(document.querySelectorAll("#objectList ul li.selected"))
  ...
  if (this.selectedApplet !== null) {
    var selector =
      '#objectList ul li[data-ar-scene-id="' + this.selectedApplet.id + '"]';
...
App.prototype.deleteApplet = function (arScene) {
  ...
  var domElement = document.querySelector(
    '#objectList ul li[data-ar-scene-id="' + arScene.id + '"]'
  );
...
App.prototype.createApplet = function (sceneType) {
  ...
  if (this.trackingBackend === "arjs") {
    var averageMatrix =
      THREEx.ArMultiMarkerControls.computeCenter(multiMarkerFile);
    averageMatrix.decompose(
      demoRoot.position,
      demoRoot.quaternion,
      demoRoot.scale
    );
  ...
  function onDelete(event) {
    ...
    // delete one scene
    var lastDomElement = [].slice
      .call(document.querySelectorAll("#objectList ul li"))
      .pop();
    if (lastDomElement) {
  ...
  function UIbuildSceneLabel(sceneType) {
    ...
    var allLabels = [].slice
      .call(document.querySelectorAll("#objectList ul li .name"))
...
App.prototype.appletsToJSON = function () {
  ...
  $.ajax({
    ...
    data: JSON.stringify(jsonData.applets),
  }).done(function (msg) {
    alert("Data Saved: " + msg);
  });
  console.log("here" + jsonData.applets);
...
App.prototype.appletsFromJSON = function (jsonData) {
  ...
  jsonData.applets.forEach(function (item) {
    ...
    parentObject3D.matrix.decompose(
      parentObject3D.position,
      parentObject3D.quaternion,
      parentObject3D.scale
    );
  });
};
//////////////////////////////////////////////////////////////////////////////
